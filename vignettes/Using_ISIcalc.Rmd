---
title: "Using the ISIcalc package"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Using the ISIcalc package}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

Below are some practical example showing how one can use the `ISIcalc` function to 
...
`ISIcalc` will load like any standard `R` package:
...
The data above has one observation per row, which is the ideal format to apply the `ISIcalc` functions quickly.
